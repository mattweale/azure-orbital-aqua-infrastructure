...
resource "azurerm_virtual_machine_extension" "cse_vm_orbital_rtstps_config" {
  ...
  settings = <<SETTINGS
    {
        "commandToExecute":"export AQUA_NFS_SHARE=${azurerm_storage_account.sa_aqua_common.name} && export T2B_SA_NAME=${data.azurerm_storage_account.sa_data_collection.name} && export AQUA_MI_ID=${azurerm_user_assigned_identity.ua_mi_orbital.client_id} && export AQUA_TOOLS_SA=${data.azurerm_storage_account.sa_aqua_tools.name} && ./main_rtstps.sh > ./logfile.txt exit 0",
